...
export default class TestOnlyLoad extends Test {
    ...
    onTest() {
        ...
        if (this.testFlag) {
            ...
            //若要释放则需要手动调用释放接口
            Resource.loadSpriteFrame("resources", "content", (err, spriteFrame) => { this.testLabel.string = "只加载 加载完成" })
            this.testLabel.node.stopAllActions()
        } else {
            Resource.loadSpriteFrame("resources", "content", (err, spriteFrame) => { Resource.decRef(spriteFrame); this.testLabel.string = "只加载 释放完成" })
            this.testLabel.node.stopAllActions()
